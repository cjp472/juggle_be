...
public class AuthFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain filterChain)
        ...
        try {
            UserSession.Authorize<UserInterface> auth = authService.readUserProfile(token);
            if (null != auth) {
                UserSession.setAuthorize(auth);
            }
            else {
                UserSession.setAuthorize(null);
                ApiResponse resp = ApiResponse.make(StandardErrors.UNAUTHORIZED.getStatus(), "User not found or your token has time out, please login again.", null);
            ...
            ex.printStackTrace();
            ApiResponse resp = ApiResponse.make(StandardErrors.INTERNAL_ERROR.getStatus(), "Failed to load user profile, please try later.", null);
            response.getWriter().write(JsonUtils.writeValue(resp));
            response.getWriter().flush();
            return;
        }
